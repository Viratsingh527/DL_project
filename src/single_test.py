...
def main_worker(args, use_gpu=True):
    ...
    mask_tf = transforms.Compose(
        [
            transforms.Resize(
                size=args.image_size, interpolation=transforms.InterpolationMode.NEAREST
            ),
            transforms.ToTensor(),
        ]
    )
    ...
    save_image(
        ...
        os.path.join(args.outputs, f"{image_name}_{mask_name}_all.png"),
        nrow=3,
        normalize=True,
    )
    save_image(
        image_masked,
        os.path.join(args.outputs, f"{image_name}_{mask_name}masked.png"),
        normalize=True,
    )
    save_image(
        comp_imgs,
        os.path.join(args.outputs, f"{image_name}_{mask_name}_comp.png"),
        normalize=True,
    )
