...
class Contrastive(nn.Module):
    def __init__(
        self, args, netD, no_mlp=False, input_dim=1024, mlp_dim=256, temp=0.07
    ):
        ...
        if not no_mlp:
            self.mlp = nn.Sequential(
                *[nn.Linear(input_dim, mlp_dim), nn.ReLU(), nn.Linear(mlp_dim, mlp_dim)]
            )
    ...
    def __call__(self, input, predict_result, gt, mask, num_feat_layers):
        ...
        textural_loss = 0.0
        print("Number of feature layers:", num_feat_layers)
        print("Length of feat_gt_final:", len(feat_gt_final))

        assert num_feat_layers <= len(feat_gt_final)
        for idx in range(1, num_feat_layers):
            feat_output, feat_gt, feat_corrupted = (
                feat_outputs[idx],
                feat_gts[idx],
                feat_corrupteds[idx],
            )
...
class dualadv:
    ...
    def D(self, netD, fake, real, masks):
        ...
        if self.SCAT_type == "nsgan" or self.SCAT_type == "lsgan":
            ...
            d_loss_SCAT_fake_valid = (
                self.relu(1.0 - d_fake_pixel) * masks
            ).mean()  # valid regions in fake
            d_loss_SCAT_fake_generated = (
                self.relu(1.0 + d_fake_pixel) * (1 - masks)
            ).mean()  # generated regions in fake
    ...
    def G(self, netD, fake, masks):
        ...
        if self.SCAT_type == "nsgan" or self.SCAT_type == "lsgan":
            ...
            g_loss_SCAT = (
                self.SCAT_lossfn(g_fake_pixel, real_label) * (1 - masks)
            ).mean()
        elif self.SCAT_type == "hingegan":
