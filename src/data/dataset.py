...
class InpaintingData(Dataset):
    def __init__(self, args):
        ...
        if args.scan_subdirs:
            ...
            self.paths = [
                entry.path
                for entry in os.scandir(args.dir_image)
        ...
        if args.transform == "randomcrop":
            ...
            raise NotImplementedError(
                "Image transformation type %s is not implemented!" % args.transform
            )
    ...
    def __getitem__(self, index):
        ...
        if self.mask_type == "pconv":
            ...
            # print(f"Mask shape before transformation: {mask.size}")
            mask = mask.convert("L")
            mask = self.mask_trans(mask)
            # print(f"Mask shape after transformation: {mask.size()}")
        ...
        # assert image.size(1) == mask.size(1) and image.size(2) == mask.size(
        #     2
        # ), f"Image and mask dimensions do not match: {image.size()} vs {mask.size()}"
        # print(f"Image shape after transformation: {image.size()}")
        # print(f"Mask shape after transformation: {mask.size()}")
