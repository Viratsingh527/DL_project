...
class Trainer:
    def __init__(self, args):
        ...
        self.contrastive_loss = getattr(loss_module, "Contrastive")(
            args, self.netD, args.no_mlp
        )
        ...
        if args.distributed:
            ...
            if not args.no_mlp:
                self.contrastive_loss.mlp = DDP(
                    self.contrastive_loss.mlp,
        ...
        if not args.no_mlp:
            self.optimG = torch.optim.Adam(
                ...
                + list(self.contrastive_loss.mlp.parameters()),
                lr=args.lrg,
    ...
    def load(self):
        try:
            gpath = sorted(
                list(glob(os.path.join(self.args.save_dir, "ckpt", "G*.pt")))
            )[-1]
        ...
        try:
            dpath = sorted(
                list(glob(os.path.join(self.args.save_dir, "ckpt", "D*.pt")))
            )[-1]
        ...
        try:
            opath = sorted(
                list(glob(os.path.join(self.args.save_dir, "ckpt", "O*.pt")))
            )[-1]
    ...
    def save(
        ...
        if self.args.global_rank == 0:
            ...
            if isinstance(self.netG, DDP):
                torch.save(
                    ...
                    os.path.join(
                        self.args.save_dir,
                        "ckpt",
                        f"G{str(self.iteration).zfill(7)}.pt",
                    ),
                )
            else:
                torch.save(
                    ...
                    torch.save(
                        {
                            ...
                            "MLP": self.contrastive_loss.mlp.module.state_dict(),
                        },
                        os.path.join(
                            self.args.save_dir,
                            "ckpt",
                            f"D{str(self.iteration).zfill(7)}.pt",
                        ),
                    )
                else:
                    torch.save(
                    ...
                    torch.save(
                        {
                            ...
                            "MLP": self.contrastive_loss.mlp.state_dict(),
                        },
                        os.path.join(
                            self.args.save_dir,
                            "ckpt",
            ...
            torch.save(
                ...
                os.path.join(
                    self.args.save_dir, "ckpt", f"O{str(self.iteration).zfill(7)}.pt"
                ),
            )
    ...
    def train(self):
        ...
        num_epoch_iter = len(self.dataloader) // (
            self.args.batch_size * self.args.world_size
        )
        ...
        for idx in pbar:
            ...
            D_losses[f"global_advd"], D_losses[f"scat_advd"] = self.dualadv_loss.D(
                self.netD, comp_img, images, 1 - masks
            )
            ...
            G_losses[f"global_advg"], G_losses[f"scat_advg"] = self.dualadv_loss.G(
                self.netD, comp_img, 1 - masks
            )
            G_losses[f"global_advg"], G_losses[f"scat_advg"] = (
                self.args.adv_weight * G_losses[f"global_advg"],
                self.args.adv_weight * G_losses[f"scat_advg"],
            )

            # contrastive learning losses
            textural_loss, semantic_loss = self.contrastive_loss(
                images_masked, comp_img, images, masks, 5
            )
